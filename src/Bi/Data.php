...
class Data
{
    ...
    function auth()
    {
        ...
        if (isset($_COOKIE['login']) && isset($_SESSION['login'])) {
            if ($_COOKIE['login'] == $_SESSION['login']) {
                ...
                $session_time = (int) $GLOBALS['settings']['session_time'] * 1;
                if ($session_time == 0) {
    ...
    function grand_access($uid, $reflink = '')
    {
        ...
        $session_time = (float) $GLOBALS['settings']['session_time'] * 1;
        if ($session_time == 0) {
    ...
    function chk_ip($redirect)
    {
        ...
        if ($bipfile) {
            if ((!file_exists($bipfile))) {
                $this->html->notFound("bipfile is missing");
                exit;
            }
        ...
        if ($use_wip) {
            ...
            if (count($allowed_ips) == 0) {
                $this->html->notFound("wipfile empty");
                exit;
            }
    ...
    function get_array($table, $field, $id)
    {
        ...
        // echo $this->html->pre_display($data,"data $table, $field, $id");
        if ($data==""){
            $data="[]";
        }
    ...
    function DB_change($what, $id, $actname = 'CHANGE')
    {
        ...
        if ($actname != 'DELETE') {
            ...
            $GLOBALS['record_old_vals'] = (is_array($GLOBALS['record_old_vals'])) ? $GLOBALS['record_old_vals'] : [];
            $GLOBALS['record_new_vals'] = (is_array($GLOBALS['record_new_vals'])) ? $GLOBALS['record_new_vals'] : [];
    ...
    function show_cal_m($year, $month)
    {
        ...
        else echo "ERR: no $class_file in " . FW_DIR;
